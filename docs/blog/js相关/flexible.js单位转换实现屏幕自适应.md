---
title: flexible.js单位转换实现屏幕自适应
createTime: 2024/12/28 23:21:14
permalink: /article/gyj6qe45/
tags:
    - js
---
...
主要是通过计算将单位px转换成rem来实现屏幕自适应。

**注意:此方案使用中不能使用rem单位所有单位必须是px。** 
...
module.exports = {
  plugins: [
    pxtorem({
      rootValue: 192,//配置分辨率下的基准值 1920
      unitPrecision: 5,
      propList: ["*"], //匹配所有属性
      selectorBlackList: [/^\.nop2r/, /^\.am/,'html'],
      //排除antd样式,html也排除在外不做rem转换
      replace: true,
      mediaQuery: false,
      minPixelValue: 0
    })
...
> 在项目src/utils目录下的增加flexible.js文件，代码如下。在main.js中引入 
>
> ````javascript
